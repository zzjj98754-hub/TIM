...
| ZooKeeper 注册/Watch | IMPLEMENTED | `RegistryZK`, `ZKit`, Gateway `ServerCache`; `/im/servers/{serverId}` 临时节点，空集群递归创建根路径且并发创建有保护 |
| Redis 用户路由 | IMPLEMENTED | Gateway 登录只返回候选节点；仅已认证 Netty Session 通过 Lua 原子写入 route Hash/TTL/presence，断开按 session/epoch 原子匹配删除 |
| 单聊可靠投递 | IMPLEMENTED-LOCALLY | `ReliableMessageService`、MySQL `im_message/outbox_event/message_delivery`、RocketMQ node topic、目标节点本地 Channel、ACK/重试；数据库条件租约、过期接管及重启后尝试上限由 H2 仓储测试覆盖，双节点 TCP 宕机恢复仍待运行验证 |
...
| 普通群写扩散 | IMPLEMENTED | `GroupMessageService`, `group_message_inbox`；批量写成员索引，正文不按成员复制，消息/Inbox/Outbox 同事务 |
| 超大群读扩散 | IMPLEMENTED | `GroupMessageService`, `group_member_cursor`；单份正文与成员读取游标，Redis 投影失败时 MySQL 权威拉取 |
...
| 业务线程池隔离与指标 | IMPLEMENTED | `BeanConfig` 有界 `ArrayBlockingQueue`、显式拒绝策略；Actuator 暴露线程池、消息计数、持久化 Delivery/Outbox backlog 与群扩散耗时 |

...
运行时依赖 Redis、MySQL、ZooKeeper 和 RocketMQ。ZooKeeper 的 `/im/servers/{serverId}` 为临时节点；Redis 路由使用 `tim:route:user:{userId}` Hash，离线索引使用 `im:offline:{userId}` ZSet；私聊节点投递使用 `TIM_NODE_MESSAGE`，群广播使用 `TIM_GROUP_BROADCAST`。
